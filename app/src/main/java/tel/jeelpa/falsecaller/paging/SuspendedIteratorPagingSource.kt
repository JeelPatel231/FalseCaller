...
class SuspendIteratorPagingSource<TData : Any>(
    iterator: SuspendIterator<Page<TData>>
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, TData> {
        ...
        val items = cachedPages.get(pageNumber).items
...
fun <T : Any> getPager(iterator: SuspendIterator<Page<T>>, pageSize: Int = 10): Pager<Int, T> {
    return Pager(
        config = PagingConfig(
            pageSize = pageSize,
            enablePlaceholders = false
        ),
        pagingSourceFactory = { SuspendIteratorPagingSource(iterator) }
    )
}
